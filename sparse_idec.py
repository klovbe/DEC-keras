...
def autoencoder(dims, act='relu', init='glorot_uniform'):
    ...
    mask_0 = K.cast(K.equal(x, 0), dtype='float32')
    mask_1 = K.cast(K.greater(x, 0), dtype='float32')
    # mask_0 = K.equal(x, 0)
    # mask_1 = K.greater(x, 0)
    ...
    mask_layer_0 = Lambda(lambda x: x*mask_0, name='mask_0')
    mask_layer_1 = Lambda(lambda x: x*mask_1, name='mask_1')
    y_0 = mask_layer_0(y)
    y_1 = mask_layer_1(y)
    # mask_layer_0 = Lambda(lambda x: tf.reshape(tf.boolean_mask(x, mask_0),[-1,1]), name='mask_0')
    # mask_layer_1 = Lambda(lambda x: tf.reshape(tf.boolean_mask(x, mask_1),[-1,1]) , name='mask_1')
    # y_0 = mask_layer_0(y)
    # y_1 = mask_layer_1(y)
    return Model(inputs=x, outputs=[y_0, y_1], name='AE'), Model(inputs=x, outputs=h, name='encoder')
...
def autoencoder_bn(dims, act='relu', init='glorot_uniform'):
    ...
    y_0 = Lambda(lambda x: x*mask_0, name='mask_0')(y)
    y_1 = Lambda(lambda x: x*mask_1, name='mask_1')(y)
    # mask_layer_0 = Lambda(lambda x: tf.reshape(tf.boolean_mask(x, mask_0),[-1,1]), name='mask_0')
    # mask_layer_1 = Lambda(lambda x: tf.reshape(tf.boolean_mask(x, mask_1),[-1,1]) , name='mask_1')
    # y_0 = mask_layer_0(y)
    # y_1 = mask_layer_1(y)
...
def wmse(y_true, y_pred):
    return K.sum(K.square(y_pred - y_true), axis=-1) / K.sum(K.cast(K.greater(y_true, 0), dtype='float32'), axis=-1)
def wmae(y_true, y_pred):
    # s = K.sum(K.cast(K.greater(y_pred, 0), dtype='float32'),axis=-1)
...
class s_IDEC(object):
    def __init__(self,
        ...
        # print(self.model)
    def pretrain(self, x, loss=None, optimizer='adam', epochs=200, batch_size=256, gamma_s = 0.05):
        print('...Pretraining...')
        self.autoencoder.compile(optimizer=optimizer, loss={'mask_0': 'mae', 'mask_1': 'mse'},
                     loss_weights=[gamma_s, 1])
        # begin pretraining
        t0 = time()
        y0 = np.zeros_like(x)
        self.autoencoder.fit(x, [y0, x], batch_size=batch_size, epochs=epochs)
    ...
    def fit(self, x_train, x_val, x_test, model_name, outdir, df_columns, y = None, epoch=500,
        ...
        if y is not None:
            ...
            print('kmans: acc = %.5f, nmi = %.5f, ari = %.5f' % (acc, nmi, ari))
        ...
        for ite in range(int(epoch)):
            ...
            while True:
                ...
                loss, cluster_loss, sparse_loss, mse_loss = self.model.train_on_batch(x=x_train[idx], y=[p[idx], y0, x_train[idx]])
            ...
            print("epoch {}th train, train_loss :{:.6f}, cluster_loss: {:.6f}, mse_loss: {:.6f}, sparse_loss: {:.6f}\n".format(ite + 1,
...
if __name__ == "__main__":
    ...
    Idec = s_IDEC(dims=[x_train.shape[-1],  300, 100, 30, 10], n_clusters=n_clusters, gamma=args.gamma, bn=args.bn)
    #
    if args.ae_weights is None:
        Idec.pretrain(x=x_test, optimizer=optimizer,
    ...
    Idec.model.summary()
    t0 = time()
    Idec.compile(optimizer=optimizer, loss={'clustering': 'kld', 'mask_0': 'mae', 'mask_1': 'mse'},
