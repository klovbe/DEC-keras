...
class Autoencoder():
    ...
    def ae_v_build(self):
        ...
        self.autoencoder_v.save_weights('{}/autoencoder_pretrain.h5'.format(outdir))

    def train_ae(self):
        self.autoencoders.compile(optimizer='adam', loss=[weighted_mse, weighted_mae])
        checkpoint = ModelCheckpoint(filepath=outdir+'best.h5', monitor='loss', save_best_only=True, save_weights_only=True)
        self.history = self.autoencoders.fit_generator(batch_generator(train_set, batch_size=batch_size
                                            , shuffle=True, beta=1.0, gamma=gamma), steps_per_epoch=steps_per_epoch,
                                            epochs=epochs_ae, callbacks=[checkpoint])
        self.autoencoder_v.save_weights('{}/autoencoder_pure.h5'.format(outdir))

    def load_pretrain_weights(self):
        self.autoencoder_v.load_weights('{}/autoencoder_pretrain.h5'.format(outdir))
    ...
    def plot_loss(self):
        ...
        f = plt.figure()
        plt.plot(self.history.history['subtract_1_loss'])
        plt.title('mse loss')
        plt.ylabel('loss')
        plt.xlabel('epoch')
        plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_iters_ae', default=4000, type=int)
    parser.add_argument('--n_iters_pretrain', default=2000, type=int)
    ...
    dims = [train_set.shape[1], 500, 500, 2000, 10, 2]
