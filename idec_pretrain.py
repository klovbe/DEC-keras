...
if __name__ == "__main__":
    ...
    parser.add_argument('--lw_epochs', default=200, type=int)
    ...
    for n_in, n_out in zip(dims[:-1], dims[1:]):
        ...
        ae = Sequential()
        if i == 0:
            ae.add(Dense(n_in, n_out, activation='relu', name='encoder_%d' % i))
            ae.add(Dense(n_out, n_in, name='decoder_%d' % i))
        elif i==3:
            ae.add(Dense(n_in, n_out, name='encoder_%d' % i))
            ae.add(Dense(n_out, n_in, activation='relu', name='decoder_%d' % i))
        else:
            ae.add(Dense(n_in, n_out, activation='relu', name='encoder_%d' % i))
            ae.add(Dense(n_out, n_in, activation='relu', name='decoder_%d' % i))
        ae.compile(loss='mean_squared_error', optimizer='rmsprop')
        ae.fit(X_train_tmp, X_train_tmp, batch_size=args.batch_size, nb_epoch=args.lw_epochs)
        # Store trainined weight
        trained_encoders.append(ae.layers[0])
        trained_decoders.append(ae.layers[1])
        # Update training data
        encoder = Model(ae.input, ae.layers[0].output)
